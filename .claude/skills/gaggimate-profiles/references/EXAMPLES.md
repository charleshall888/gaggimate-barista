...
The **Automatic Pro** is a firmware built-in profile by modsmthng_57901. It uses flow-variable pressure — targeting a flow rate with a pressure ceiling so extraction adapts to puck resistance automatically.

**Best for:** All roasts, beginners, consistency without constant adjustment

**Key technique for custom profiles:**
- Use `"target": "flow"` with a `"pressure"` ceiling — the machine self-regulates
- Scale flow to dose: `Flow = Dose × 2 / 20s` (16g → 1.6, 18g → 1.8, 22g → 2.2 g/s)
- Add declining flow via linear transition for sweetness (e.g., 40s decline from brew flow → 1.2 g/s)
- Use low pressure ceilings (1-2 bar) for fill/saturation phases, 12 bar for ramp, 9 bar for extraction

> **Full firmware profile documentation**: See [`automatic-pro/AUTOMATIC_PRO_GUIDE.md`](../../knowledge/automatic-pro/AUTOMATIC_PRO_GUIDE.md) for the 5-phase vIT3 architecture, dose scaling tables, and phase-by-phase analysis.
>
> **Technique reference**: See [`FLOW_VARIABLE_PRESSURE.md`](./FLOW_VARIABLE_PRESSURE.md) for dose scaling formulas, advantages, and troubleshooting.
...
| **Automatic Pro** (firmware) | 91°C | Variable (≤9) | 30-40s | All roasts, consistency |
