...
The foundational "why" behind espresso extraction — what happens when water meets ground coffee, and the physics that determine whether that meeting produces deliciousness or disappointment.
...
The enemy of even extraction. Understanding channeling physics helps prevent it.

### The Physics: A Positive Feedback Loop
...
The order of impact on channeling prevention:

1. **Distribution (most important)**
   - WDT (Weiss Distribution Technique) breaks up clumps and distributes grounds evenly
   - Even distribution means even density means uniform flow resistance
   - No amount of tamping can fix uneven distribution

2. **Tamping (important but secondary)**
   - Creates a level surface and compresses the bed uniformly
   - Consistent pressure matters more than high pressure (15–30 lbs is fine)
   - A level tamp prevents the "high side" from channeling

3. **Pressure profile (tertiary)**
   - Lower pressure is more forgiving of prep imperfections
   - Gentle pressure ramp allows the puck to "set" before full pressure
   - Pre-infusion at low pressure saturates the entire bed before extraction force is applied

### Puck Prep Tools and What Each Does
...
2. If you apply full pressure to a dry puck, water flows through any weak spot it can find → instant channeling
...
- This allows water to fully penetrate all particles
...
→ *For shot-by-shot troubleshooting, see the diagnose skill or `GAGGIMATE_PROFILE_CREATION_GUIDE.md` troubleshooting section*
...
*This document covers the "why" of extraction. For practical guidance on adjustments, see `ESPRESSO_BREWING_BASICS.md`. For pressure selection, see `PRESSURE_GUIDE.md`. For profile creation, see `GAGGIMATE_PROFILE_CREATION_GUIDE.md`.*
